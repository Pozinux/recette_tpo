...
printf "%-45s %25s" "### Creation fichier csv avec volumetrie" " $h-FS.csv ... "
printf "/ 2G\n/boot 2G\n/usr 4G\n/opt 1G\n/tmp 2G\n/var 2G\n/var/log 2G\n/var/log/audit 2G\n/home 1G\n/comp/rear 3,5G\n/comp 8G" >$RECETTE_REMOTE_REP/$h-FS.csv
printf "%-10s\n" " $OK"
...
if [ $virtual -eq 1 ]; then
  ...
  if [ $? -eq 1 ]; then
     ...
     stat="NOK"
     # Pour les RHEL 7, la maj des tools est gérée par l'OS et non pas par VMware donc il indique 'guestToolsUnmanaged'. Signifie qu'il y a les tools quand même. Voir KB VMWARE 1032763.
     cat $VMWARE_HWVERSION_FILE | grep -i $h | grep -i -e 'guestToolsCurrent' -e 'guestToolsUnmanaged' >/dev/null 2>&1
     [[ $? -eq 0 ]] && stat="OK"
...
stat="NOK"
cat /etc/redhat-rel* | grep '7.5' >/dev/null 2>&1
[[ $? -eq 0 ]] && stat="OK"
...
[[ $tmp && $res ]] && gateway=$(netstat -rn | grep UG | grep $res | head -1 | awk -F' ' '{print $2}')
stat="NOK"
ping -c1 -w1 $gateway >/dev/null 2>&1
[[ $? -eq 0 ]] && stat="OK"
# cas de la passerelle 0.0.0.0
[[ $gateway == "*" ]] && stat="OK"
printf "%-45s %25s %-50s\n" "### Check IP SVC (ping gateway)" "$gateway ... " $stat
...
# cas de la passerelle 0.0.0.0
[[ $gateway == "*" ]] && stat="OK"
printf "%-45s %25s %-50s\n" "### Check IP ADM (ping gateway)" "$gateway ... " $stat
...
if [ $ip_nas != "-" ]; then
  ...
  [[ "$stat" == $WARN ]]
  printf "%-45s %25s %-50s\n" "### Check IP NAS (ping gateway)" "$gateway ... " $stat
else
...
stat="NOK"
res=$(chronyc tracking | grep status | awk -F' ' '{print $(NF)}')
[[ $res == "Normal" ]] || [[ $res == "synchronised" ]] && stat="OK"
printf "%-40s %30s %-50s\n" "### Check NTP" " ... " $stat
...
stat="OK"
checkpubkeys=" cat /home/$user_ssh_rhel7/.ssh/authorized_keys | grep -i"
checkpubkeysinvalides="$checkpubkeys -v"
checkpubkeystotal=" cat /home/$user_ssh_rhel7/.ssh/authorized_keys | grep '^ssh' | wc -l"
...
####### Check /srce dans auto.srce
## Note : En rhel 7 ce n'est plus dans auto.direct et l'autofs n'est utilisé que pour la phase de déploiement. A la fin le durcissement le désactive.

# stat="WARN"
# grep "/srce" /etc/auto.srce >/dev/null 2>&1
# [[ $? -eq 0 ]] && stat="OK"
# printf "%-50s %20s %-50s\n" "### Check /srce dans auto.srce" " ... " $stat


####### Check montage /srce 

stat=$WARN
mount p1-nas000.marium.int:/SRCE /srce >/dev/null 2>&1
...
stat="NOK"
i=0
for user in $(cat /etc/passwd | grep -e '/bin/' | grep -v -e $user_ssh_rhel7 -e root -e sync | awk -F':' '{ print $1}'); do
...
entete_once=0
for i in $(df -PBM | grep -v -i -e '/srce' -e '/shm' -e 'Filesystem' -e 'tmpfs'); do
